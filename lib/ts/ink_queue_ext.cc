...
mmap_align(size_t size, size_t alignment)
{
  ...
  if (result == MAP_FAILED) {
    ...
    ink_fatal(1, "Failed to mmap %zu bytes, %s", size, (errno == ENOMEM) ? err_str : strerror(errno));
  }
...
get_chunk_item_magic_idx(InkFreeList *f, void *item, InkChunkInfo **ppChunk, bool do_check = false)
{
  ...
  if (do_check && (idx >= f->chunk_size || ((uintptr_t)item - chunk_addr) % f->type_size)) {
    ink_stack_trace_dump();
    ink_fatal(1, "Invalid address:%p, chunk_addr:%p, type_size:%d, chunk_size:%u, idx:%u", item, (void *)chunk_addr, f->type_size,
              f->chunk_size, idx);
  }
