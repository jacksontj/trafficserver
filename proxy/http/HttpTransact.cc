...
is_header_keep_alive(const HTTPVersion &http_version, const HTTPVersion &request_http_version,
                     MIMEField *con_hdr /*, bool* unknown_tokens */)
{
  enum { CON_TOKEN_NONE = 0, CON_TOKEN_KEEP_ALIVE, CON_TOKEN_CLOSE };
  ...
  if (HTTPVersion(1, 0) == http_version) {
    ...
    //   server doesn't
    keep_alive =
      ((con_token == CON_TOKEN_KEEP_ALIVE) || (con_token == CON_TOKEN_NONE && HTTPVersion(1, 1) == request_http_version)) ?
        (HTTP_KEEPALIVE) :
        (HTTP_NO_KEEPALIVE);
  } else {
...
HttpTransact::initialize_state_variables_from_response(State *s, HTTPHdr *incoming_response)
{
  ...
  if ((s->current.request_to != ORIGIN_SERVER) &&
      (s->current.request_to == PARENT_PROXY || s->current.request_to == ICP_SUGGESTED_HOST)) {
...
{
  enum KA_Action_t { KA_UNKNOWN, KA_DISABLED, KA_CLOSE, KA_CONNECTION };
