...
enum FTP_MODE { FTP_NULL, FTP_PORT, FTP_PASV };
...
UrlHashTable::UrlHashTable()
{
  ...
  if (*url_hash_filename) {
    ...
    bytes = (unsigned char *)mmap(NULL, numbytes, PROT_READ | PROT_WRITE, MAP_SHARED | MAP_NORESERVE, fd, 0);
  ...
  *eheaders = 0;
  ...
  if (proxy_port)
    sprintf(fd[sock].req_header, "GET %s HTTP/1.0\r\n"
                                 ...
                                 "%s"
                                 "\r\n",
            curl, reload_rate > drand48() ? "Pragma: no-cache\r\n" : "",
            fd[sock].keepalive ? "Proxy-Connection: Keep-Alive\r\n" : "", eheaders);
  else
    sprintf(fd[sock].req_header, "GET /%s%s%s%s%s HTTP/1.0\r\n"
                                 ...
                                 "%s"
                                 "\r\n",
            path, xquer ? "?" : "", quer, xpar ? ";" : "", para, host, reload_rate > drand48() ? "Pragma: no-cache\r\n" : "",
            fd[sock].keepalive ? "Connection: Keep-Alive\r\n" : "", eheaders);
