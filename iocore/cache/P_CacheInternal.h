                                 ...
                                 char *hostname, int host_len)
{
  ...
  if (cache_clustering_enabled > 0) {
    return open_read_internal(CACHE_OPEN_READ_BUFFER, cont, buf, (CacheURL *)0, (CacheHTTPHdr *)0, (CacheLookupHttpConfig *)0, key,
                              0, frag_type, hostname, host_len);
  }
#endif
  return caches[frag_type]->open_read(cont, key, frag_type, hostname, host_len);
}
...
TS_INLINE Action *
CacheProcessor::open_write_buffer(Continuation *cont, MIOBuffer *buf, CacheKey *key, CacheFragType frag_type, int options,
...
TS_INLINE Action *
CacheProcessor::open_read_buffer(Continuation *cont, MIOBuffer *buf, URL *url, CacheHTTPHdr *request, CacheLookupHttpConfig *params,
                                 CacheFragType type)
{
  (void)buf;
#ifdef CLUSTER_CACHE
  if (cache_clustering_enabled > 0) {
    return open_read_internal(CACHE_OPEN_READ_BUFFER_LONG, cont, buf, url, request, params, (CacheKey *)0, 0, type, (char *)0, 0);
  }
#endif
...
TS_INLINE Action *
CacheProcessor::open_write_buffer(Continuation *cont, MIOBuffer *buf, URL *url, CacheHTTPHdr *request, CacheHTTPHdr *response,
                                  CacheFragType type)
{
  (void)cont;
  (void)buf;
