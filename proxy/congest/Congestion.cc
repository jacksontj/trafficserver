...
#define IsGt0(var)                                                                                      \
  if (var < 1) {                                                                                        \
    ...
    snprintf(error_buf, error_len, "line %d: invalid %s = %d, %s must > 0", line_num, #var, var, #var); \
    cleanup();                                                                                          \
    return error_buf;                                                                                   \
  }
  ...
  if (max_connection_failures >= CONG_RULE_MAX_max_connection_failures ||
    ...
    error_buf = (char *)ats_malloc(error_len);
    snprintf(error_buf, error_len, "line %d: invalid %s = %d not in [1, %d) range", line_num, "max_connection_failures",
