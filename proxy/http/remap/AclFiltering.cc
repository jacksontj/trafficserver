...
acl_filter_rule::acl_filter_rule()
  : next(NULL), filter_name_size(0), filter_name(NULL), allow_flag(1), src_ip_valid(0), active_queue_flag(0), argc(0)
{
...
acl_filter_rule::print(void)
{
  int i;
  printf("-----------------------------------------------------------------------------------------\n");
  printf("Filter \"%s\" status: allow_flag=%d, src_ip_valid=%d, active_queue_flag=%d\n", filter_name ? filter_name : "<NONAME>",
         (int)allow_flag, (int)src_ip_valid, (int)active_queue_flag);
