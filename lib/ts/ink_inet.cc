...
REGRESSION_TEST(Ink_Inet)(RegressionTest *t, int /* atype */, int *pstatus)
{
  ...
  // Test ats_ip_parse() ...
  {
    ...
    for (unsigned i = 0; i < countof(names); ++i) {
      ...
      box.check(ats_ip_parse(ts::ConstBuffer(names[i].hostspec, strlen(names[i].hostspec)), &addr, &port) == 0, "ats_ip_parse(%s)",
                names[i].hostspec);
      box.check(strncmp(addr.data(), names[i].host, addr.size()) == 0, "ats_ip_parse(%s) gave addr '%.*s'", names[i].hostspec,
                (int)addr.size(), addr.data());
      if (names[i].port) {
        box.check(strncmp(port.data(), names[i].port, port.size()) == 0, "ats_ip_parse(%s) gave port '%.*s'", names[i].hostspec,
                  (int)port.size(), port.data());
      } else {
        box.check(port.size() == 0, "ats_ip_parse(%s) gave port '%.*s'", names[i].hostspec, (int)port.size(), port.data());
      }
    }
  }
