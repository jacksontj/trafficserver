                                           ...
                                           void * /*arg ATS_UNUSED */)
{
  ...
  Debug("ssl", "creating NPN list: spdy on npnSet: %p", netvc->npnSet);
...
int
SSLNetVConnection::advertise_next_protocol_no_spdy(SSL *ssl, const unsigned char **out, unsigned int *outlen,
                                           void * /*arg ATS_UNUSED */)
{
  SSLNetVConnection * netvc = (SSLNetVConnection *)SSL_get_app_data(ssl);
  ...
  // Here we prevent spdy from being offered.
  Debug("ssl", "creating NPN list: spdy off npnSet: %p", netvc->npnSet);
  if (netvc->npnSet && netvc->npnSet->advertiseProtocolsNotPrefixed(out, outlen, "spdy", 4)) {
    // Successful return tells OpenSSL to advertise.
    return SSL_TLSEXT_ERR_OK;
  }
...
int
SSLNetVConnection::select_next_protocol_no_spdy(SSL * ssl, const unsigned char ** out, unsigned char * outlen, const unsigned char * in ATS_UNUSED, unsigned inlen ATS_UNUSED, void *)
{
  SSLNetVConnection * netvc = (SSLNetVConnection *)SSL_get_app_data(ssl);
  const unsigned char * npn = NULL;
  unsigned npnsz = 0;
  ...
  if (netvc->npnSet && netvc->npnSet->advertiseProtocolsNotPrefixed(&npn, &npnsz, "spdy", 4)) {
    // SSL_select_next_proto chooses the first server-offered protocol that appears in the clients protocol set, ie. the
    // server selects the protocol. This is a n^2 search, so it's preferable to keep the protocol set short.
